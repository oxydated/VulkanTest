...
bool isDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface) {
	...
	if (extensionsSupported) {
		...
		swapChainAdequated = !swapChainSupport.formats.empty() && !swapChainSupport.presentModes.empty();
	}

	VkPhysicalDeviceFeatures supportedFeatures{};
	vkGetPhysicalDeviceFeatures(device, &supportedFeatures);

	return indices.isComplete() && extensionsSupported && swapChainAdequated && supportedFeatures.samplerAnisotropy;
}
...
void generateMipmaps(VkPhysicalDevice physicalDevice, VkDevice device, VkCommandPool commandPool, VkQueue graphicQueue, VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
	...
	endSingleTimeCommands(device, commandBuffer, graphicQueue, commandPool);
}
...
void updateUniformBuffer(uint32_t currentImage, VkExtent2D extent, std::vector<void*>& buffersMapped) {
